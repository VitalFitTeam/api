...
// ------------------------------
// GET /schedule/:classId/bookings
// ------------------------------

// @Summary		Get class bookings
// @Description	Retrieves all bookings for a specific class with client information
// @Tags			Schedule
// @Security		ApiKeyAuth
// @Produce		json
// @Param			classId	path		string								true	"Class UUID"
// @Success		200		{object}	object{data=[]ClassBookingResponse}	"List of bookings"
// @Failure		400		{object}	map[string]interface{}				"Invalid class ID"
// @Failure		500		{object}	map[string]interface{}				"Server error"
// @Router			/schedule/{classId}/bookings [get]
func (h *ScheduleHandlers) GetClassBookingsHandler(c *gin.Context) {
	ctx := c.Request.Context()
	...
	bookings, err := h.services.ScheduleServices.GetClassBookings(ctx, classID)
	if err != nil {
		h.services.LogErrors.InternalServerError(c, err)
		return
	}

	// Convert domain bookings to response format
	response := make([]*ClassBookingResponse, 0, len(bookings))
	for _, booking := range bookings {
		response = append(response, &ClassBookingResponse{
